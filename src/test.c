...
void test_poisson_distribution(int n, int k, int seed)
{
    ...
    for (; get_length(a) != n;
         ++number)
    { // start loop to add numbers to dynamic array
    ...
    struct QDigest *q = qdigestCreate(
    ...
    {
        _insert(q, b->data[i], 1, true, S); // insert data inside the QDigest
    }
...
void test_random_distribution(int n, int k, int seed)
{
    ...
    srand(seed);
    struct QDigest *q = qdigestCreate(k, 1);
...
void test_merge(int n, int k, int seed)
{
    printf("<< test_merge >>\n");
    srand(seed);
    struct QDigest *q1 = qdigestCreate(k, 1);
    struct QDigest *q2 = qdigestCreate(k, 1);
    ...
    for (int i = 0; i < n * 2; ++i)
    {
        ...
        if (i < n)
        {
            _insert(q1, number, 1, true, 6);
        }
        else
        {
            _insert(q2, number, 1, true, 6);
        }
    }
